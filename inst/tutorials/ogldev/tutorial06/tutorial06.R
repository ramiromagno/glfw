...
render_scene <- function(window, vbo, g_translation_location) {
  ...
  m <- glm_translate(m = diag(nrow = 4), v = c(scale * 2, scale, 0, 1))

  gl_uniform_matrix4_fv(g_translation_location,
                        transpose = GL$`TRUE`,
                        value = t(m))
...
compile_shaders <- function(vs_file, fs_file) {
  ...
  g_translation_location <- gl_get_uniform_location(shader_program, "gTranslation")
  if (g_translation_location == -1L)
    stop("Error getting uniform location of 'gTranslation'")
...
window <- glfw_create_window(width, height, "Tutorial 06: translation transformation")
...
g_translation_location <-
  compile_shaders(
    vs_file = system.file("tutorials/ogldev/tutorial06/shader.vs", package = "glfw"),
    fs_file = system.file("tutorials/ogldev/tutorial06/shader.fs", package = "glfw")
  )
...
while (!glfw_window_should_close(window))
{
  ...
  render_scene(window, vbo, g_translation_location)
  glfw_poll_events()
}
