...
lighting_vs <- system.file("tutorials/learnopengl/colors/colors.vs", package = "glfw")
lighting_fs <- system.file("tutorials/learnopengl/colors/colors.fs", package = "glfw")
lighting_program <- build_program(vert = lighting_vs, frag = lighting_fs)

light_cube_vs <- system.file("tutorials/learnopengl/colors/light_cube.vs", package = "glfw")
light_cube_fs <- system.file("tutorials/learnopengl/colors/light_cube.fs", package = "glfw")
...
lighting_loc <- list()
lighting_loc[['objectColor']] <- gl_get_uniform_location(lighting_program, "objectColor")
lighting_loc[['lightColor']] <- gl_get_uniform_location(lighting_program, "lightColor")
lighting_loc[['projection']] <- gl_get_uniform_location(lighting_program, "projection")
lighting_loc[['view']] <- gl_get_uniform_location(lighting_program, "view")
lighting_loc[['model']] <- gl_get_uniform_location(lighting_program, "model")

light_cube_loc <- list()
light_cube_loc[['objectColor']] <- gl_get_uniform_location(light_cube_program, "objectColor")
light_cube_loc[['lightColor']] <- gl_get_uniform_location(light_cube_program, "lightColor")
...
while (!glfw_window_should_close(window))
{
  ...
  gl_use_program(lighting_program)
