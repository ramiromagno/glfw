...
vs <- system.file("tutorials/learnopengl/coord_systems/coord_systems.vs", package = "glfw")
fs <- system.file("tutorials/learnopengl/coord_systems/coord_systems.fs", package = "glfw")
...
while (!glfw_window_should_close(window))
{
  ...
  model <- diag(1, nrow = 4)
  view <- diag(1, nrow = 4)
  projection <- diag(1, nrow = 4)

  model <- glm_rotate(model, glfw_get_time(), c(0.5, 1, 1))
  view <- glm_translate(view, c(0, 0, -3))
