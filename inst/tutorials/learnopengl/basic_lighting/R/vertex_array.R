setup_vao <- function(vertices) {
  ...
  gl_buffer_data(GL$ARRAY_BUFFER, t(vertices), GL$STATIC_DRAW)

  gl_bind_vertex_array(cube_vao)
  gl_vertex_attrib_pointer(0L, 3L, GL$DOUBLE, GL$`FALSE`, 6L, 0L)
  gl_enable_vertex_attrib_array(0L)

  gl_vertex_attrib_pointer(1L, 3L, GL$DOUBLE, GL$`FALSE`, 6L, 3L)
